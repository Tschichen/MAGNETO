...
class DirGraphAlign(object):
    ...
    def __init__(self, G1, G2, node_score_list=None, edge_score_list=None, forbid=False):
        ...
        if self.node_score_list is not None:
        ...
        self.allowed_labels = self.find_allowed_matches()
    ...
    def find_allowed_matches(self):
        """Return dict with allowed node matches from node label list"""
        if self.node_score_list:
            allowed_label_dict = dict()
            for label_list in self.node_score_list:
                label_1 = label_list[0]
                allowed_matches = set()
                for i in range(0, len(label_list[1])):
                    if not math.isinf(label_list[1][i][1]):
                        allowed_matches.add(label_list[1][i][0])
                        allowed_label_dict.update({label_1: allowed_matches})
            return allowed_label_dict
        else:
            return 0
    ...
    def cand_iter(self):
        ...
        deg_key = self.G2_node_degorder.__getitem__
        ...
        if self.G1.is_directed():
            ...
            if T1_out and T2_out:
                if self.node_score_list:
                    node_2 = self.no_lone_node(T1_out, T2_out)
                else:
                    node_2 = T2_out[0]
                ...
                if T1_in and T2_in:
                    if self.node_score_list:
                        node_2 = self.no_lone_node(T1_in, T2_in)
                    else:
                        node_2 = T2_in[0]
                    if node_2 is not None:
                        for node_1 in T1_in:
            ...
            if T1_inout and T2_inout:
                if self.node_score_list:
                    node_2 = self.no_lone_node(T1_inout, T2_inout)
                else:
                    node_2 = T2_inout[0]
                if node_2 is not None:
    ...
    def no_lone_node(self, nodeset1, nodeset2):
        for node2 in nodeset2:
            ...
            for node1 in nodeset1:
                ...
                if label2 in self.allowed_labels[label1]:
                    return node2
        else:
            return None
    ...
    def syntactic_feas(self, G1_node, G2_node):
        ...
        feasi = True
        if self.G1.is_directed():
            feasi = self.synfeas_dir(G1_node, G2_node)
        else:
            feasi = self.syntactic_feas_undir(G1_node, G2_node)
        return feasi

    def synfeas_dir(self, G1_node, G2_node):
    ...
    def syntactic_feas_undir(self, G1_node, G2_node):
        for neigh in self.G1[G1_node]:
            if neigh in self.core_1:
                if not (self.core_1[neigh] in self.G2[G2_node]):
                    return False
                elif self.G1.number_of_edges(neigh, G1_node) != self.G2.number_of_edges(self.core_1[neigh], G2_node):
                    return False
                elif self.match_is_forbidden(self.G1[neigh][G1_node]['Label'],
                                             self.G2[self.core_1[neigh]][G2_node]['Label'], self.forbidden_edges):
                    return False

        for neigh in self.G2[G2_node]:
            if neigh in self.core_2:
                if not (self.core_2[neigh] in self.G1[G1_node]):
                    return False
                elif self.G1.number_of_edges(self.core_2[neigh], G1_node) != self.G2.number_of_edges(neigh,
                                                                                                     G2_node):
                    return False
                elif self.match_is_forbidden(self.G2[neigh][G2_node]['Label'],
                                             self.G1[self.core_2[neigh]][G1_node]['Label'], self.forbidden_edges):
                    return False
    ...
    def process_results(self, results):
        ...
        if len(pruned_Res) > 1:
            ...
            self.printOutput(pruned_Res[0])
            return (pruned_Res[0])

    def getkey(self, element):
        return (element[1])

    def printOutput(self, table):
